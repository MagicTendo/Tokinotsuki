...
const { getValue, hasValue, updateValue } = require("./database.js");

async function addTeamPoints(interaction, userID, points, response = true) {
    const { flagToTeam } = require("./flags.js");
    const getTeam = await getValue(userID, "users", "team");

    if (getTeam) {
        const teamName = await flagToTeam(getTeam);
...
async function completeQuest(userID, quest) {
    if (await hasValue(userID, "users", `quest-${quest}`))
        return false;

    await updateValue(userID, "users", `quest-${quest}`, 1, false);
...
function getCurrencySymbol(currency) {
    switch (currency) {
        case "congelo":
            return "<:CongeloPoint:1462204702287728701>";
...
async function getEmoteList() {
    const emotes = {
        "\\n": "\n",
        "[Banane]": "[Banane](<https://www.youtube.com/watch?v=3r7mIB3Rhw4>)",
        "[Coolux]": "\n```　    ∧,,∧\n　 (；・ω・）　　,\n　 /　ｏ={=}ｏ , ', ´\n､､しー-Ｊミ(.@)ｗｗｗｗｗｗｗｗｗｗｗ```\n",
        "[TokiAngry]": "💢",
        "[TokiConfused]": "🌀",
        "[TokiCookie]": "🍪",
...
async function getPing(interaction, fullReport = false, isRefresh = false) {
    const sentMessage = await interaction.reply({ content: "<a:Loading:1400419944847376438> Calcul en cours...", withResponse: true });
    const ping = sentMessage.resource.message.createdTimestamp - interaction.createdTimestamp;

    if (isRefresh)
        await sentMessage.resource.message.delete();
    ...
    if (fullReport) {
        ...
        const speedAppreciations = ["Trop rapide 🚀", "Très rapide 🛹", "Rapide ⚡", "Lent 🕓", "Très lent 🐌"];
        let speedScore;
        ...
        return [ping, apiPing, speedAppreciations[speedScore]];
    } else {
        return ping;
    }
}
...
function getRandomKanji() {
    ...
    const numberKanji = kanji.length;
    const randomKanjiIndex = Math.floor(Math.random() * numberKanji);

    return [kanji[randomKanjiIndex], randomKanjiIndex, numberKanji];
}

async function getWinningTeam(userID) {
    ...
    for (let i = 0; i < teams.length; i++) {
        ...
        if (teamPoints > winningPoints) {
            ...
            isTie = false;
        } else if (teamPoints === winningPoints) {
            winningTeamFlag = 0;
            winningTeam = "tie";
            isTie = true;
        }
    ...
    return { "flag": winningTeamFlag, "name": winningTeam, "isUserTeamWinning": !isTie && userTeam === winningTeamFlag };
}

function isLink(link) {
    return link.match(/(https?:\/\/(?:www\.)?)?[-a-zA-Z0-9@%._\+~#=]{1,256}\.[a-z]{2,6}\b(?:[-a-zA-Z0-9@:%_\+.~#?&\/\/=]*)/gm);
}

async function simplify(userID, number, forceConversion = false) {
    if (!forceConversion && await hasValue(userID, "users", "unsimplify"))
        return Number(number).toLocaleString().replaceAll(",", " ");
...
async function updateMemberCounts(client) {
    const server = client.guilds.cache.find(guild => guild.id === process.env.GUILD_COMMANDS_ID);
    const botCount = String(server.members.cache.filter(member => member.user.bot).size);
    const memberCount = String(server.memberCount - botCount);
    const memberCountChannel = server.channels.cache.find(channel => channel.id === process.env.MEMBER_COUNT_CHANNEL_ID);
    const botCountChannel = server.channels.cache.find(channel => channel.id === process.env.BOT_COUNT_CHANNEL_ID);
    const memberCountChannelPrefixName = memberCountChannel.name.split(":")[0];
    const botCountChannelPrefixName = botCountChannel.name.split(":")[0];

    await memberCountChannel.setName(`${memberCountChannelPrefixName} : ${memberCount}`);
    await botCountChannel.setName(`${botCountChannelPrefixName} : ${botCount}`);
}

module.exports = { addTeamPoints, capitalize, clean, completeQuest, getCurrencySymbol, getEmoteList, getPing, getRandomKanji, getWinningTeam, isLink, simplify, updateMemberCounts };